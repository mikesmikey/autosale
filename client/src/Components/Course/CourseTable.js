...
class CourseTable extends Component {
  ...
  dataMainUpdate () {
    ServiceObj.getAllSubject().then((usersData) => {
      if (this._isMounted) {
  ...
  loadDataBySubjectID (subjid, subjname) {
    ServiceObj.searchAllSubjectBySubjectIdOrSubjectName(subjid, subjname).then((usersData) => {
      if (this._isMounted) {
        //this.props.setDataLoadingStatus(false)
        this.setState({ data: usersData })
        this.setState({ Input: subjname })
        // console.log(this.state.data)
      }
    })
  }
  ...
  loadDataIntoTable () {
    var returnData = []
    var select = document.getElementById(this.props.FacultySelect)
    for (var i = 0; i < this.state.data.length; i++) {
  ...
  render () {
    return (
      <table className="room-table" id="SubjectTable" >
        <thead>
